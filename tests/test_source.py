...
def test_create_source(connection):
    hook = SourcesAPI(connection)
    response = hook.create_source(source_name="new_pg", workspace_id=workspace_id, source_definition_id='decd338e-5647-4c0b-adf4-da0e75f5a750', params={"host": "localhost", "port": 5432, "username": "postgres", "password": "password", "database": "postgres", "schemas": ["public"], "ssl_mode": {"mode": "disable"}, 
    "tunnel_method": {"tunnel_method": "NO_TUNNEL"}, "replication_method": {"method": "Standard"}, "jdbc_url_params": ""})
    print (response.json())